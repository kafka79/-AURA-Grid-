    ...
    if (activeAlerts.length === 0) {
      this.container.innerHTML = `
        <div class="empty-alerts">
          ...
          <span style="font-weight: 500; color: var(--text-secondary)">All Systems Operating Normallly</span>
          <span style="font-size: 0.7rem; color: var(--text-muted)">Smart Grid AI optimization active.</span>
        </div>
      `;
      return;
    }
    ...
    this.container.innerHTML = activeAlerts.map(alert => `
      <div class="alert-item ${alert.level}">
        <div class="alert-item-header">
          <span class="alert-title flex-between" style="gap: 6px; color: ${alert.level === 'critical' ? 'var(--color-red)' : 'var(--color-amber)'}">
